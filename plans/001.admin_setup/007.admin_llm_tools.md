# Feature 001 Step 007 — Admin LLM Tools
...
**Lore strategy when tools are enabled.** Non-injected lore facts are not included in context — the LLM fetches them actively via `search`/`get_lore`. However, `is_injected=True` lore facts are always present (see step 007b — `007b.lore_injection.md`). Rationale: injecting all lore creates a huge, mostly irrelevant context blob; active fetching forces the LLM to be deliberate and produces better consistency checks.
