...
**Lore NOT injected when tools are enabled.** Previously, all world lore was concatenated into the system prompt. With tools enabled, this is skipped entirely. The LLM is instructed to fetch what it needs actively. Rationale: injecting all lore creates a huge, mostly irrelevant context blob; active fetching forces the LLM to be deliberate and produces better consistency checks.
